...
def BM25(collection: bow_document_collection, query: dict) -> dict:
    ...
    if len(collection.docs) == 0:
        raise AttributeError(
            "collection: object contains no documents (bow_document objects)."
        )
    ...
    for query_term, query_frequency in query.items():
        n_i = collection.term_doc_count.get(
            query_term, 0
        )  # the number of documents containing term i (0 if not present)

        # Calculate the inverse document frequency for the term
        idf_component = math.log10(
            ((r_i + 0.5) / (R - r_i + 0.5))
        ...
        for doc_id, doc in collection.docs.items():
            ...
            document_term_frequency = doc.terms.get(
                query_term, 0
            )  # query term frequency within the document (0 if not present)

            # Calculate the term frequency normalization for the document term
            saturation_component = ((k_1 + 1) * document_term_frequency) / (
                K + document_term_frequency
            )
            ...
            score = (
                idf_component * saturation_component * query_component
            )  # determine the score

            doc_scores[doc_id] += (
                score  # update the document's score with the product of the IDF and TF components
            )
...
def JM_LM(collection: bow_document_collection, query: dict) -> dict:
    ...
    if len(collection.docs) == 0:
        raise AttributeError(
            "bow_document_collection: object contains no documents (bow_document objects)."
        )
    ...
    for query_term in query:
        ...
        for doc_id, doc in collection.docs.items():
            ...
            p_coll = (
                (term_collection_frequency / total_corpus_length)
    ...
    doc_scores = {
        doc_id: (score if updated_docs[doc_id] else 0)
...
def cosine_similarity(vector1: list, vector2: list) -> float:
    ...
    return (
        dot_product / (norm1 * norm2) if (norm1 and norm2) != 0 else 0.0
    )  # return similarity, handling 0 case for a Euclidean norm of 0
...
def vector_space_model(collection: bow_document_collection, query: dict) -> dict:
    ...
    collection_term_frequency = (
        collection.term_frequency
    )  # dict of the total number of times each term appears in collection
    N = len(collection.docs)  # total number of documents in the collection
    ...
    return dict(
        sorted(
            document_similarity_scores.items(), key=lambda item: item[1], reverse=True
        )
    )


def w5(
    collection: bow_document_collection, evaluations: dict, theta: int | float = 0
) -> dict:
    """
    ...
    N = len(collection.docs)  # total number of documents in the collection
    R = sum(
        1 for relevancy in evaluations.values() if relevancy == 1
    ...
    for doc_id, doc in collection.docs.items():
        for term in doc.terms.keys():
            if evaluations[doc_id] == 1:
                term_relevance_count[term] = (
                    term_relevance_count.get(term, 0) + 1
                )  # initialise term with 1 if not present, otherwise add 1
            term_total_count[term] = (
                term_total_count.get(term, 0) + 1
            )  # initialise term with 1 if not present, otherwise add 1
    ...
    for term, relevance_count in term_relevance_count.items():
        ...
        score_denominator = (n_tk - relevance_count + 0.5) / (
            N - n_tk - R + relevance_count + 0.5
        )
    ...
    selected_features = {
        term: score
        for term, score in weight5_scores.items()
        if score > mean_weight5 + theta
    }
...
def My_PRM(
    weighting_function,
    collection: bow_document_collection,
    query: dict,
    threshold: int,
    theta: int,
) -> dict:
